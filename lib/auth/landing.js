...
/**
 * The store this account runs, if it runs one: an ACTIVE store, or one Campus
 * Dash has SUSPENDED.
 *
 * A SUSPENDED STORE STILL HAS AN OWNER AT ITS COUNTER. Suspension takes it off
 * the marketplace; it does not take the owner out of their dashboard, where
 * the orders already placed still have to be made and handed over. The
 * database agrees — is_vendor_staff() is ownership, not store status — and
 * every write that would put the store back on sale refuses a store that is
 * not ACTIVE. `vendor_ids` stays what it was: the stores that can trade.
 */
export function vendorStoreId(capabilities) {
  if (capabilities?.vendor_ids?.length) return capabilities.vendor_ids[0];
  if (capabilities?.vendor_status === 'SUSPENDED' && capabilities.vendor_id) {
...
export function landingFor(capabilities) {
  ...
  if (vendorStoreId(capabilities)) return '/vendor';
...
export function areasFor(capabilities) {
  ...
  if (vendorStoreId(capabilities)) areas.push({ href: '/vendor', label: 'Vendor' });

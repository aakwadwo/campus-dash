...
/**
 * The graduation years worth offering.
 *
 ...
 *
 * Computed from the current date rather than hard-coded so this does not have
 * to be edited every September. Six years forward covers a first-year on a
 * long programme; one year back covers somebody finishing this academic year
 * who signs up in January.
 */
 ...
 */
export function validateSignUpDetails(details, now = new Date()) {
  ...
  if (affiliation === 'STUDENT') {
    ...
    const allowed = graduationYears(now);
    if (!allowed.includes(graduationYear)) {
      return { ok: false, error: 'Choose the year you expect to graduate.' };
    }
  }

  // OPTIONAL, AND STAYS OPTIONAL. Nobody is stopped from buying lunch for
  // declining to say.
  if (gender && !GENDERS.includes(gender)) {
    return { ok: false, error: 'Choose male or female, or leave it blank.' };
  }

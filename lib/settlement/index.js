 ...
 *
 * TWO DIFFERENT RUNS, AND ONLY ONE OF THEM SENDS ANYTHING.
 *
 *   VENDOR   transfers, when a store has no Paystack subaccount to split to.
...
export async function runSettlement({ payeeType, periodStart, periodEnd }) {
  ...
  //
  // VENDORS ARE UNAFFECTED. Their share is split at the charge; this run is the
  // fallback for a store with no subaccount, and it still transfers.
  const manual = payeeType === 'PARTNER';

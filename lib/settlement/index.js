...
import { partnerWeek } from './payday';
import {
 ...
 *
 * TWO DIFFERENT RUNS, AND NEITHER SENDS ANYTHING ON THIS DEPLOYMENT.
 *
 *   VENDOR   for a store with no Paystack subaccount to split to. Transfers
 *            when the provider can send them; otherwise, like a Partner run,
 *            creates the payouts and stops (settlesByHand()).
 *   PARTNER  creates the payouts and stops. An administrator sends the money
 *            and records the reference; see runSettlement() below and
...
/**
 * The period a run settles.
 *
 * VENDORS: the calendar day before `endingAt`.
 *
 * PARTNERS: the week that ended at the most recent Sunday 00:00, Accra time
 * (which is UTC, all year). Sunday is payday, so the week is Sunday to
 * Saturday and it is the SAME week whichever day the run is gathered on — a
 * run pressed on Monday settles the week that closed on Sunday, not a week
 * that ends on Monday. create_settlement_run() returns an existing run for a
 * period rather than making a second, so gathering twice is harmless.
 */
export function periodFor(payeeType, endingAt = new Date()) {
  if (payeeType === 'PARTNER') return partnerWeek(endingAt);
...
/**
 * Whether a run for this payee type stops at "gathered, pay by hand".
 *
 * PARTNERS: always. That is the policy — see runSettlement().
 *
 * VENDORS: when the provider cannot send transfers on this deployment. The run
 * used to try anyway, and sendPayout() failed every payout on the spot, which
 * released the money back to "owed" and left nothing an administrator could
 * record as paid (a FAILED payout cannot be settled by hand). Gathering it into
 * a PENDING payout instead is the Partner model: the store is paid externally
 * and the payment is recorded with its reference. Vendors paid by Paystack
 * split never reach a run at all.
 */
export function settlesByHand(payeeType, provider = getPaymentProvider()) {
  return payeeType === 'PARTNER' || !provider.canSendTransfers;
}
...
export async function runSettlement({ payeeType, periodStart, periodEnd }) {
  ...
  // PARTNERS ARE SETTLED BY HAND, BY DESIGN.
  //
  // This used to call sendPayout() for every payee and rely on
  // PAYSTACK_TRANSFERS_ENABLED being false to stop a Partner transfer actually
  // leaving. An environment variable is not a policy: it is one deploy away
  // from being wrong, it is invisible to anybody reading this function, and the
  // thing it was preventing is money leaving automatically to a destination
  // nobody had checked.
  //
  // So a Partner run CREATES the payouts and stops. They sit PENDING until an
  // administrator sends the money and records the reference, through
  // admin_settle_payout_manually(), which appends to admin_actions.
  //
  // The ledger is identical either way: allocations are claimed by the run and
  // settled when the payout is, so "owed" and "settled" mean what they always
  // meant and admin_pending_settlement() still answers "where is the money now".
  //
  // VENDORS: their share is split at the charge; this run is the fallback for a
  // store with no subaccount. It transfers only where transfers are actually
  // possible, and otherwise gathers the payouts for an administrator to pay by
  // hand, exactly like a Partner run — see settlesByHand().
  const manual = settlesByHand(payeeType);
  ...
  return {
    runId: run.id,
    payeeType,
    // "Nothing was sent, and that is correct" has to be distinguishable from
    // "nothing was sent because something broke".
    manual,
    awaitingManualSettlement: manual ? (payouts?.length ?? 0) : 0,
 ...
 *
 * It cannot return PAID, and that is the whole point. Hard rule 15 says

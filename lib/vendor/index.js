 ...
 */
/**
 * A PRICE THE CUSTOMER CHOOSES is `pricing`: { mode, minPesewas, stepPesewas,
 * maxPesewas, clearMax, choicesPesewas }, and absent means "not touching it".
 * Choosing STEPPED or CHOICES, or changing either rule, is refused in SQL for a
 * store Campus Dash has not given the capability to — whatever this sends.
 */
function pricingArgs(pricing) {
  if (!pricing) return {};
  return {
    p_pricing_mode: pricing.mode ?? null,
    p_variable_min_pesewas: pricing.minPesewas ?? null,
    p_variable_step_pesewas: pricing.stepPesewas ?? null,
    p_variable_max_pesewas: pricing.maxPesewas ?? null,
    p_variable_choices_pesewas: pricing.choicesPesewas ?? null,
  };
}
...
export function updateMenuItem({
  ...
  return rpc('vendor_update_menu_item', {
    ...
    ...pricingArgs(pricing),
    ...(pricing?.clearMax ? { p_clear_variable_max: true } : {}),
  });
}
...
/**
 * Where the store says it is: ON_CAMPUS or OFF_CAMPUS, and the words a
 * customer reads under the store's name. Its own call, touching those two
 * columns only, so saving a location can never change anything else about the
 * store. Owner only, checked in SQL.
 */
export function updateLocation({ vendorId, area, details }) {
  return rpc('vendor_update_location', {
    p_vendor_id: vendorId,
    p_area: area,
    p_details: details,
  });

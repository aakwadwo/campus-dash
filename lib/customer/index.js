 ...
 */
export async function listSearchableItems() {
  const supabase = await createClient();
  const { data, error } = await supabase
    .from('menu_items')
    .select('id, name, description, price_pesewas, vendor_id')
...
export async function getVendorWithMenu(vendorId) {
  ...
  const { data: menu } = await supabase
    .from('menu_items')
    // image_path is the dish's own photograph, on the PUBLIC vendor-images
    // bucket — the same one storefront photographs use, because an
    // unauthenticated visitor browsing the marketplace has to see it.
    .select('id, name, description, price_pesewas, is_available, sort_order, image_path')

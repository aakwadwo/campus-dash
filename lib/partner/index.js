...
export async function acceptDelivery(orderId) {
  ...
  if (result.success) {
    // The code is read inside the deferred task: it exists only to go in the
    // customer's SMS, so it is not the Partner's to wait for.
    await announceAfter(NOTIFICATION_EVENT.PARTNER_ASSIGNED, orderId, async () => ({
...
export async function confirmPickup(orderId, pickupCode) {
  ...
  if (result.success) {
    await announceAfter(NOTIFICATION_EVENT.PARTNER_PICKED_UP, orderId);
  }
  return result;
}
...
export async function completeDelivery(orderId, deliveryCode) {
  ...
  if (result.success) {
    await announceAfter(NOTIFICATION_EVENT.DELIVERY_COMPLETED, orderId);
  }
  return result;
}

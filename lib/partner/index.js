 ...
 *
 * Name, student ID number, level and the verified school address are already on
 * the account: PARTNER ⇒ CUSTOMER, and the database refuses this call without
 * that row. Which is also why nothing here asks for a face photograph — the
 * school address has already established who this is, and a second photograph
 * proved nothing it had not while being the most sensitive thing Campus Dash
 * held. An application adds the student ID and the Partner terms; the rest is
 * an administrator's decision.
 */
export function apply({ studentIdImagePath }) {
  return rpc('partner_apply', { p_student_id_image_path: studentIdImagePath });
}

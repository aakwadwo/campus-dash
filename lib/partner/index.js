...
/**
 * Apply to be a Partner. ONE argument, and that is the whole point.
 *
 * Student ID number, class year, email and the ID photograph are already on the
 * account: they were collected at student onboarding, which the database
 * requires before this call is allowed at all. A Partner application adds the
 * live face photograph and nothing else — it is an upgrade to the account in
 * the session, never a second identity.
 */
export function apply({ faceImagePath }) {
  return rpc('partner_apply', { p_face_image_path: faceImagePath });
}

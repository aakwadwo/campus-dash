...
export async function purgePartnerDocuments({ userId, paths, reason }) {
  ...
  //
  // `paths` is now optional. It was always filtered down to the one path this
  // function derives for itself, so a caller that supplies nothing gets exactly
  // the same target — and a screen that does not know the path (the Partner
  // detail page deliberately never receives one; admin_partner_detail returns
  // only whether a document EXISTS) can still purge without the path making a
  // round trip through the browser.
  const allowed = await partnerFacePathFor(userId);
  const targets = (paths ?? [allowed]).filter(Boolean).filter((path) => path === allowed);

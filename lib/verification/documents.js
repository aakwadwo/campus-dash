...
/**
 * A Meal Scan goes somewhere else entirely.
 *
 ...
 * at once by an administrator, whereas a Meal Scan is shown to the store that
 * is about to honour it. Keeping them apart means a policy change to one can
 * never widen the other.
 *
 * PHOTOGRAPHS ONLY, and PDF is deliberately gone. It was accepted because some
 * entitlements are issued that way, which was true and cost more than it was
 * worth: the customer could not see a preview of what they were about to send,
 * the store had to open a document viewer at a counter, and "choose a file"
 * opened a document picker on a phone when what everybody actually wanted was
 * the camera roll. A screenshot of a PDF is a photograph, and every phone takes
 * one in two taps.
 */
const SCAN_BUCKET = 'scan-documents';
const SCAN_ALLOWED = new Set(['image/jpeg', 'image/png', 'image/webp']);
...
export async function uploadVendorImage({ vendorId, file }) {
  ...
  //
  // ASKED AS THE CALLER. is_vendor_staff() answers from auth.uid(), which the
  // service-role client does not have, so asking it there said "no" to every
  // store owner and only an administrator could ever upload a photo. That is
  // why no store's picture reached the marketplace.
  const asCaller = await createClient();
  const { data: allowed, error: checkError } = await asCaller.rpc('is_vendor_staff', {
...
export async function uploadScan({ file }) {
  ...
  if (!SCAN_ALLOWED.has(file.type)) {
    throw new Error('Please use a photo of your Meal Scan — a JPEG, PNG or WebP image.');
  }
  if (file.size > MAX_BYTES) {

...
const PATTERNS = [
  ...
  // THE UNIQUENESS RULES, each said back in the words of the field that broke.
  // These messages are raised deliberately by complete_customer_onboarding()
  // and contain no table names, constraint names or ids — and a person whose
  // student ID is already registered can do nothing at all with "something went
  // wrong on our side", which is what the generic fallback would have told them.
  {
    match: /student ID number is already registered/i,
    kind: ERROR_KIND.CONFLICT,
    message:
      'That student ID number is already registered to another account. Check it, or sign in to the account you already have.',
  },

  {
    match: /phone number is already used/i,
    kind: ERROR_KIND.CONFLICT,
    message: 'That phone number is already on another Campus Dash account.',
  },

  {
    match: /email address is already used/i,
    kind: ERROR_KIND.CONFLICT,
    message: 'That email address is already on another Campus Dash account. Try signing in.',
  },

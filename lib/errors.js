...
const PATTERNS = [
  ...
  // AN ADMINISTRATOR'S CLOSE AND REOPEN (admin_set_vendor_open). Each is the
  // rule working, and each names the way forward.
  {
    match: /the store has nothing on its menu\. It opens when the store turns an item on/i,
    kind: ERROR_KIND.CONFLICT,
    message:
      'There is nothing to put back: the menu has changed since it was closed. The store opens when it turns an item on.',
  },
  {
    match: /only an ACTIVE store can be opened or closed|vendor is not active/i,
    kind: ERROR_KIND.CONFLICT,
    message:
      'Only an active store can be opened or closed. A suspended store is reinstated from its status.',
  },
  {
    match: /this store is already closed/i,
    kind: ERROR_KIND.CONFLICT,
    message: 'This store is already closed.',
  },
  {
    match: /this store is already open/i,
    kind: ERROR_KIND.CONFLICT,
    message: 'This store is already open.',
  },
  {
    match: /meal scans are not turned on for this store/i,
    kind: ERROR_KIND.FORBIDDEN,
    message: 'Meal scans are not turned on for this store.',
  },

...
import { cachedPlatformConfig } from '@/lib/customer/catalogue';
 ...
 * React's cache() is what makes "per request" true: it memoises within one
 * server render and starts empty for the next, so the layout, the page and a
 * poll interval all share one read without any value outliving the request.
 */
...
const FALLBACK = Object.freeze({
  ...
  // The switches. Delivery defaults ON so a database that cannot be reached
  // degrades into offering the normal product rather than quietly withdrawing
  // half of it; the pack fee defaults to 0 so a screen never invents a charge.
  partner_delivery_enabled: true,
  scan_pack_fee_pesewas: 0,
});

export const getPlatformConfig = cache(async () => {
  try {
    // Shared across requests and expired the moment an administrator saves a
    // change (admin_update_config). See lib/customer/catalogue.js.
    const data = await cachedPlatformConfig();

...
export class ArkeselSmsProvider extends SmsProvider {
   ...
   *
   *   `trace` is a TEMPORARY injected logger — see lib/observability/otp-trace.js.
   *   Injected rather than imported so this module keeps its one rule: nothing
   *   Arkesel-specific leaks out, and nothing environmental leaks in.
  ...
  async send(phoneNumber, message, options = {}) {
    ...
    let response;
    let body;
    // ONE request. There is no retry loop in this method and there must not be:
    // a resend that Arkesel already accepted costs a second SMS, and on the OTP
    // path a second SMS is a second chance to confuse the customer.
    const startedAt = Date.now();
    trace('arkesel.request.start', { hasCallback: Boolean(callbackUrl), timeoutMs });
    try {
      ...
      trace('arkesel.response.headers', {
        http: response.status,
        elapsedMs: Date.now() - startedAt,
      });
      body = await response.text();
      trace('arkesel.response.body', {
        bytes: body.length,
        elapsedMs: Date.now() - startedAt,
      });
    } catch (error) {
      ...
      trace('arkesel.request.failed', { timedOut, elapsedMs: Date.now() - startedAt });
      return this.#failure(
        timedOut ? `Arkesel did not respond within ${timeoutMs}ms` : 'could not reach Arkesel',
        { correlationId, retryable: true, elapsedMs: Date.now() - startedAt }
      );
    }
    ...
    trace('arkesel.parsed', { code: code ?? 'none', elapsedMs: Date.now() - startedAt });
    ...
    return {
      ...
      correlationId,
      providerCode: 'ok',
      // How long Arkesel took to ACCEPT. Not how long delivery took — nothing
      // observable here can tell us that; the delivery report can.
      elapsedMs: Date.now() - startedAt,
  ...
  #failure(
    error,
    { correlationId = null, providerCode = null, retryable = false, elapsedMs = null } = {}
  ) {
    return {
      ok: false,
      accepted: false,

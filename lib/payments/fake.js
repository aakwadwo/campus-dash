...
export class FakePaymentProvider extends PaymentProvider {
  ...
  /**
   * A stand-in subaccount code, shaped like Paystack's so nothing downstream
   * has to know which provider issued it. Deterministic on the account number,
   * so restarting the dev server does not invent a second destination for the
   * same phone.
   */
  async ensureSubaccount({ businessName, momoNetwork, accountNumber }) {
    if (!businessName) throw new Error('ensureSubaccount requires a business name');
    if (!accountNumber) throw new Error('ensureSubaccount requires an account number');
    const code = `ACCT_fake_${momoNetwork.toLowerCase()}_${accountNumber}`;
    console.log(`[fake-payment] subaccount ${code} for ${businessName}`);
    return { subaccountCode: code, raw: { business_name: businessName, subaccount_code: code } };
  }
  ...
  async initiateCollection({
    ...
    if (existingId) {
      ...
      return {
        ...
        redirectUrl: null,
        splitApplied: Boolean(existing.split),
        splitError: null,
    ...
    const txn = {
      ...
      reference,
      metadata,
      // Recorded rather than acted on. There is no real settlement here, but a
      // developer exercising the order flow should still see the ledger say
      // SPLIT where a real Paystack account would have split — otherwise the
      // one path that is never tried locally is the one that moves money.
      split: split?.subaccountCode ? { ...split } : null,
    ...
    return {
      ...
      status: 'PENDING',
      redirectUrl: null,
      splitApplied: Boolean(txn.split),
      splitError: null,
      raw: { ...txn },
    };
  }

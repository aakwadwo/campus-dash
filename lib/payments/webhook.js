...
async function applyTransferEvent(event, providerName) {
  ...
  if (event.status === 'SUCCEEDED') {
    ...
    // PAID is the first moment there is anything true to tell somebody. A
    // transfer the provider merely accepted is PROCESSING, and telling a vendor
    // their money had arrived at that point would sometimes be a lie.
    //
    // After the response: the payout is PAID the moment the call above returns,
    // and the provider is owed its 200 then rather than after an SMS round trip.
    const { notifyPayoutSent } = await import('@/lib/notifications/dispatch');
    const { deferNotification } = await import('@/lib/notifications/defer');
    await deferNotification('PAYOUT_SENT', () => notifyPayoutSent(payout.id));

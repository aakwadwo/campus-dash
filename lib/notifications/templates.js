...
export const SMS_TEMPLATES = {
  ...
  // THE ONE MESSAGE THE CUSTOMER CANNOT AFFORD TO MISS. Everything else that
  // happens to a Meal Scan order shows up on the tracking page they are
  // watching; this one ends the order while they are not watching, tells them
  // the money is not coming back, and asks them to do something about it.
  //
  ...
  // accuses somebody of something in a message they cannot reply to. The
  // tracking page carries the store's wording where there is room for it.
  [E.SCAN_REFUSED]: {
    [A.CUSTOMER]: (c) =>
      `Campus Dash: ${c.vendorName} could not accept the Meal Scan on order #${c.orderNumber}. The order is cancelled and cannot be refunded. Please order again with a valid Meal Scan.`,
  },

...
export const SMS_TEMPLATES = {
  ...
  // Acceptance no longer means "now pay": the customer still has to say whether
  // they are collecting it or having it brought, and the price depends on the
  // answer. Naming an amount here would name the wrong one.
  [E.ORDER_ACCEPTED]: {
    [A.CUSTOMER]: (c) =>
      `Campus Dash: ${c.vendorName} accepted order #${c.orderNumber}. Choose pickup or delivery in the app, then pay.`,
  },
  ...
  [E.ORDER_READY]: {
    [A.CUSTOMER]: (c) =>
      c.isPickup
        ...
        : `Campus Dash: order #${c.orderNumber} is ready. We are finding a Partner to bring it.`,
  },
  ...
  [E.PARTNER_ASSIGNED]: {
    ...
    [A.PARTNER]: (c) =>
      `Campus Dash: order #${c.orderNumber} from ${c.vendorName} is yours. You earn ${formatPesewas(c.earningsPesewas)}. Details in the app.`,
  },
  ...
  [E.DELIVERY_COMPLETED]: {
    ...
    [A.PARTNER]: (c) =>
      `Campus Dash: order #${c.orderNumber} completed. ${formatPesewas(c.earningsPesewas)} added to your earnings. Thank you.`,
  },

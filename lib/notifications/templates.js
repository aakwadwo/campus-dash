...
export const SMS_TEMPLATES = {
  ...
  // press Ready.
  [E.PAYMENT_CONFIRMED]: {
    // NAMES THE FOOD, NOT THE NUMBER. A queue number is the right identifier
    // inside the app, where it sits beside the order it names and beside the
    // store calling it out. On a lock screen it is a thing to go and look up,
    // so this says what they bought instead. The number is still on the order
    // screen, one tap away, and is unchanged.
    [A.CUSTOMER]: (c) =>
      `Campus Dash: payment confirmed. ${c.vendorName} is preparing your ${c.orderSummary}.`,
    [A.VENDOR]: (c) =>
...
export function renderSms(event, audience, context) {
  ...
  return template ? toGsm7(template(context)) : null;
}

...
export async function notify({
  ...
  const results = await Promise.all(
    recipients.map(async ({ audience, phone, userId = null }) => {
      ...
      // A notification is identified by what it IS: this event, about this
      // subject, to this recipient. Server actions retry and pages revalidate,
      // and without this check each of those could put a second identical SMS
      // in somebody's pocket — which costs real money and reads, to them, like
      // the system is broken.
      //
      // The subject is usually the order. It is not always: a vendor approval
      // or a payout is about a store or a payout id, and passing one of those
      // as dedupeSubject is what stops a double-clicked admin button sending
      // the same congratulations twice.
      //
      // AN EXPLICIT SUBJECT WINS OVER THE ORDER ID, and that is what makes a
      // re-broadcast possible. When a Partner cancels, the order genuinely
      // needs carrying again and every eligible Partner should hear about it —
      // including the ones who were told the first time and did not take it.
      // Keyed on the order alone, that second broadcast deduplicated against
      // the first and reached nobody. The caller passes
      // `<orderId>:gen:<dispatch_generation>` instead, so a new round of
      // offers is a genuinely new notification while a retried one is not.
      const subject = dedupeSubject ?? orderId;
      const dedupeKey = subject ? `${event}:${audience}:${subject}:${phone}` : null;

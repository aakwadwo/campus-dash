...
const AUDIENCES = {
  ...
  // THE CUSTOMER ONLY. The store just pressed the button and is looking at the
  // result; a Partner was never assigned, because a scan order's search does
  // not open until the scan is approved and this is the other branch.
  [NOTIFICATION_EVENT.SCAN_REFUSED]: [AUDIENCE.CUSTOMER],
...
export async function notifyOrderEvent(event, orderId, extra = {}) {
  ...
  try {
    ...
    const context = {
      ...
      appUrl: config.publicAppUrl() ?? 'https://campusdash.app',

...
export async function startPayment(orderId) {
  ...
  const split = await vendorSplitFor(order, payment, provider);
 ...
 *
 * FOOD SUBTOTAL, EXACTLY. Not a percentage of the total, which also contains
 * the service fee and the delivery fee; the vendor is owed what their food
 * costs and that is a figure the order already carries.
 *
 * Null in every case where a split would be wrong rather than merely absent:
 *
 *   * a SCAN errand — Campus Dash never sold their food and owes them nothing;
 ...
 *   * a subtotal of zero, which is not a split but a rejected request.
 *
 ...
 */
async function vendorSplitFor(order, payment, provider) {
  if (order.order_type === 'SCAN') return null;

  const subtotal = Number(order.subtotal_pesewas ?? 0);
  if (!Number.isInteger(subtotal) || subtotal <= 0) return null;
  if (subtotal > Number(payment.amount_pesewas)) return null;
  ...
  const destination = await payoutDestinationFor({ payeeType: 'VENDOR', payeeId: vendorId });
  ...
  return { subaccountCode: destination.provider_subaccount_code, sharePesewas: subtotal };
}

 ...
 *
 * The client sends menu item ids, quantities and a fulfilment choice, plus a
 * chosen unit price for an item whose price the customer picks. That price is
 * checked against the item's own rule and refused if it is not on it; every
 * other figure is read from the menu and pricing_config and snapshotted.
 */
export async function submitOrder({
  ...
  const rows = await callAsUser('submit_order', {
    ...
    // Order information, for the store. Written with the order, before payment.
...
async function announce(result, event, orderId) {
  // AFTER THE RESPONSE. The transition above has committed; the SMS is a
  // consequence of it and must not hold the button. See deferNotification().
  if (result.success) await deferNotification(event, () => notifyOrderEvent(event, orderId));
  return result;
}
...
export async function confirmPayment({ paymentId, providerTransactionId, amountPesewas }) {
  ...
  if (orderId) {
    // AFTER THE RESPONSE. confirm_payment() has committed — the order is PAID and
    // in the kitchen whether or not these messages go out — so a webhook answers
    // Paystack as soon as the money is recorded, not after two SMS round trips.
    await deferNotification(NOTIFICATION_EVENT.PAYMENT_CONFIRMED, async () => {

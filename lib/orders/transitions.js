...
/**
 * A vendor and some items. That is the whole submission.
 *
 * Pickup or delivery is NOT decided here — it is decided after the vendor has
 * accepted, by customerChooseFulfilment below, because until then there may not
 * be an order to make the decision about.
 *
 ...
 */
export async function submitOrder({ vendorId, items }) {
...
/**
 * Pickup or delivery, after acceptance and before payment.
 *
...
/**
 * READY is also the moment dispatch opens for a delivery order — never before,
 * because a Partner should not stand at a counter waiting for food.
 *
 * So this is where the offer broadcast goes out. The recipient list is decided
 * in SQL, from the same eligibility rules the offer list uses, and the message
 * says only that work exists: see lib/notifications/dispatch.js.
 */
...
/**
 * The pickup code the VENDOR reads out to the Partner at the counter.
 *
 * The direction reversed: the vendor now holds the secret and the Partner
 * performs the act. Whoever holds it must not also be the one confirming, or
 * the code proves nothing — so the Partner has no function that returns this.
 */
export function vendorPickupCode(orderId) {
  return callAsUser('vendor_pickup_code', { p_order_id: orderId });
}

/**
 * The vendor types in the collection code the CUSTOMER shows them.
 *
 * Self-pickup is still a handoff and still needs to be more than "they said it
 * was theirs". Same mechanism as the delivery handoff, different holders.
 */
export async function vendorCompletePickupOrder(orderId, pickupCode) {
  return unwrap(
    await callAsUser('vendor_complete_pickup_order', {
      p_order_id: orderId,
 ...
 * see hard rule 11 — so the payout sits at PROCESSING until a transfer event

...
| Table                 | Purpose                                                                                                                                                                                                                                                                            |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `users`               | **IDENTITY**, one row per `auth.users` row: phone, email, name, suspension, `is_admin`. Unique phone AND unique email — `email` is real and supplied by the person, never synthesised. `is_admin` is a column here, never a client-supplied claim. Carries no capability evidence. |
| `customer_profiles`   | **The CUSTOMER capability.** A row here IS the capability — there is no flag. Student ID number, class year and the private Storage path to the ID photograph, written only by `complete_customer_onboarding()`. Admin and vendor-staff accounts do not get one automatically.     |
| `partner_profiles`    | **The PARTNER capability.** Exists only once someone applies, and only on top of a `customer_profiles` row. Holds the private Storage path to the live face photograph, the review decision, availability, and a document retention deadline.                                      |

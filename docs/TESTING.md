...
| `account-model`                          | Identity vs capability: sign-up through both doors, the school-domain rule, the Customer→Partner upgrade on one auth user, admin ⇏ customer, vendor ⇏ customer, email and phone uniqueness, and the two delivery conflict rules |
...
| `paystack`                               | The Paystack adapter, against a stub fetch. Real HMAC. Never calls Paystack                                                                                                                                                     |
| `paystack-payouts`                       | Payout lifecycle, customer email, mobile money destinations                                                                                                                                                                     |
| `paystack-splits`                        | Subaccount creation, the flat dynamic split, a refused split not stopping a payment, and the ledger channel each allocation ends up on                                                                                          |
| `rewards`                                | The order count at 24/25/39/40/49/50/51, that only COMPLETED counts, and that a second reward is impossible                                                                                                                     |
| `partner-capacity`                       | The configured maximum at 1, 2, 3 and 5, changing it mid-pilot, and the slot index refusing a direct write                                                                                                                      |
| `handoff-codes`                          | Four digits in all three directions, reversed and foreign codes, and the lockout — including that a stranger cannot spend somebody else's attempts                                                                              |
| `partner-ratings`                        | One rating per delivery, who may leave one, and that a Partner sees an average rather than rows                                                                                                                                 |
| `routes`                                 | Every major route, actually requested against a running server. Catches what no SQL test can: a page that compiles, queries fine and still 500s or renders the error boundary                                                   |
| `partner-payouts`                        | GH₵5 accumulation, the GH₵20 weekly floor at every amount either side of it, rollover across weeks, idempotency, the transfer lifecycle, and that a Partner is never shown a provider failure                                   |
| `names-and-scan-details`                 | First names in both directions and when they stop, plus the required scan details and who may read them                                                                                                                         |

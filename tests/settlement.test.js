...
describe('settlement and reconciliation', () => {
  ...
  test('a delivered order splits three ways and sums to what was paid', async () => {
    ...
    assert.equal(money.platform_allocation, 200, 'the service fee');
    assert.equal(money.allocated_pesewas, 4200);
    assert.equal(money.total_pesewas, 4200);
    assert.equal(money.balances, true);
    assert.equal(money.paid_pesewas, 4200);
  });
  ...
  test('the balance trigger refuses to let allocations drift in the first place', async () => {
    ...
    assert.match(error.message, /sum to 3500 but order total is 4200/);
  });
  ...
  test('reconciliation still catches an imbalance if one ever appeared', async () => {
    ...
    assert.match(issue.detail, /allocations sum to 3500 but the order total is 4200/);
  });
